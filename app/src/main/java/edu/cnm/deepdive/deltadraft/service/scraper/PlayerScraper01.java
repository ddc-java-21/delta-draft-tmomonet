...
public class PlayerScraper01 {
  ...
  public static List<Player> scrapePlayers() throws IOException {
    ...
    Element standardTable = extractCommentedTable(standardDoc, "players_standard_batting");
  ...
  private static Element extractCommentedTable(Document doc, String tableId) {
    return extractFromComments(doc, tableId);
  }

  private static Element extractFromComments(Node node, String tableId) {
    for (Node child : node.childNodes()) {
      if (child instanceof Comment) {
        String data = ((Comment) child).getData();
        if (data.contains("id=\"" + tableId + "\"")) {
          Document commentDoc = Jsoup.parse(data);
          Element table = commentDoc.getElementById(tableId);
          if (table != null) {
            return table;
          }
        }
      } else {
        Element result = extractFromComments(child, tableId);
        if (result != null) {
          return result;
        }
      }
